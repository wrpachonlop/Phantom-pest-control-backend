...
func (r *ClientRepository) GetByID(ctx context.Context, id uuid.UUID) (*models.ClientFull, error) {
	...
	err := r.db.QueryRow(ctx, `
		SELECT
			...
			ccd.inspector_id,
			ccd.billing_terms,
			ccd.initial_setup_cost, 
			ccd.recurring_service_cost,
			ccd.service_frequency, 
			ccd.frequency_interval,
			ccd.proposal_drive_link,
			ccd.approved_by_name, 
			ccd.approved_date,
			ccd.next_followup_date, 
			ccd.installation_date, 
			ccd.cancelled_date, 
			ccd.cancel_reason,
		...
		LEFT JOIN crew_members cw ON ccd.inspector_id = cw.id
		WHERE c.id = $1
	`, id).Scan(
		&client.ID,
		&client.ClientName,
		...
		&client.PropertyType,
		&client.Status,
		&client.ClientContactDate,
		&client.FirstContactDate,
		&client.SoldDate,
		&client.AfterHours,
		&client.ContactMethodID,
		&client.ProblemDescription,
		&client.LocationType,
		&client.LocationValue,
		&client.SoldBy,
		&client.SaleRange,
		&client.CreatedBy,
		&client.CreatedAt,
		&client.UpdatedAt,
		// contact method
		&client.ContactMethod.ID,
		&client.ContactMethod.Name,
		&client.ContactMethod.IsActive,
		&client.ContactMethod.CreatedAt,
		// commercial details
		...
		&client.Details.ApprovedDate,
		&client.Details.NextFollowupDate,
